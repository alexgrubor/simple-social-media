...
const Friends = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-orange-400 to-pink-400 h-screen flex flex-col justify-center items-center px-2 pt-20">
      ...
      <div className="flex flex-col gap-4">
        {friends &&
          friends.map((friend: any) => (
            <div
              key={friend._id}
              className="bg-gray-100 rounded-lg p-4 flex flex-col gap-2 sm:flex-row sm:items-center"
            >
              <img
                src={friend.user.profilePicture}
                alt={friend.user.firstName}
                className="w-40 h-40 object-cover rounded-full mx-auto sm:mx-0 sm:mr-4"
              />
              <div className="flex flex-col justify-center">
                <p className="text-lg font-medium mb-1">
                  {friend.user.firstName} {friend.user.lastName}
                </p>
                <p className="text-gray-500 mb-2">{friend.user.email}</p>
                <button
